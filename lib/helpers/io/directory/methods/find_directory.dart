...
Future<List<String>> findDirectoriesByName(String rootPath, String name, {bool exactMatch = true, bool ignoreHidden = true}) async {
  ...
  if (await dir.exists()) {
    await for (final entity in dir.list(recursive: true, followLinks: false)) {
      if (entity is Directory && (!ignoreHidden || (!entity.path.split(Platform.pathSeparator).last.startsWith('.')))) {
        final dirName = entity.path.split(Platform.pathSeparator).last;
        // Check for exact or partial match based on the exactMatch parameter
        final isMatch = exactMatch ? dirName.trim() == name.trim() : dirName.trim().contains(name.trim());
        if (isMatch) {
...
Future<List<String>> findDirectoriesByPattern(String rootPath, RegExp pattern, {bool ignoreHidden = true}) async {
  final matches = <String>[];
  final dir = Directory(rootPath);

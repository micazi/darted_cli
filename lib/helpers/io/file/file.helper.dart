...
class FileHelper {
  ...
  Future<List<String>> find(String rootPath, String name, {bool isExactMatch = false, bool ignoreHidden = true}) async =>
      await findFilesByName(rootPath.replaceSeparator(), name, exactMatch: isExactMatch, ignoreHidden: ignoreHidden);

  /// Search for a file with a RegExp pattern.
  Future<List<String>> findAdvanced(String rootPath, RegExp pattern, {bool ignoreHidden = true}) async => await findFilesByPattern(rootPath.replaceSeparator(), pattern, ignoreHidden: ignoreHidden);

  Future<Map<String, List<(int matchLine, String lineContent)>>> search(String rootPath, RegExp pattern, {bool ignoreHidden = true, List<RegExp> excluded = const []}) async =>
      await searchFilesContent(rootPath.replaceSeparator(), pattern, ignoreHidden: ignoreHidden, excluded: excluded);

  /// Search the content of files in a root path for a RegExp pattern.
  Future<Map<String, List<(int matchLine, String lineContent)>>> searchAndReplace(String rootPath, RegExp pattern,
          {bool ignoreHidden = true, List<RegExp> excluded = const [], String? replacement}) async =>
      await searchFilesContent(rootPath.replaceSeparator(), pattern, ignoreHidden: ignoreHidden, excluded: excluded, replacement: replacement);

  /// Creates a file at the specified path (The file's name is the last is that path).
  Future<File> create(String path, {bool createFoldersInPath = true}) async => await createFile(path.replaceSeparator(), createFoldersInPath: createFoldersInPath);
  ...
  Future<void> writeString(String path, String content, {bool rewrite = true}) async => await writeStringToFile(path.replaceSeparator(), content, rewrite: rewrite);

  /// Write content to a file as bytes (Uint8List). File will be created if it doesn't exist.
  Future<void> writeBytes(String path, Uint8List content, {bool rewrite = true}) async => await writeBytesToFile(path.replaceSeparator(), content, rewrite: true);

...
Future<bool> confirmImpl({String? prompt, List<String>? acceptedAffirmatives}) async {
  ...
  String p = prompt ?? 'Are you sure?'.withColor(ConsoleColor.blue) + '(yes/no): '.withColor(ConsoleColor.grey);
  String userInput = await getUserInputImpl((defValue, timeout) => p);
  //
  return affirvatives.map((a) => a.toLowerCase().trim()).contains(userInput.toLowerCase().trim());
}
